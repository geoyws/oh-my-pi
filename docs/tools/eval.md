...
- A managed kernel that answers SIGINT keeps running, and so do the processes that cell spawned: the runtime still owns their handles, so the next cell can manage them. Child reaping belongs to the hard kill that follows an unresponsive runtime, and only there.
- On POSIX both *subprocess* runtimes — the Python kernel and the isolated JS eval worker — are spawned detached and lead their own process group, so a hard kill (the escalation after a failed interrupt, and the JS timeout kill) sweeps that group: processes the cancelled cell spawned itself are reaped with the runtime, while processes outside the group (host-owned detached work, other sessions' runtimes) are untouched. Windows has no process groups, and the Bun Worker fallback is a thread rather than a process; both fall back to terminating the runtime alone. A hard kill of a *subprocess* runtime waits a bounded window for its exit and logs the surviving pid if it cannot confirm it. The Bun Worker fallback has no pid and no group: its termination is confirmed only by the thread's `close` event, and an unconfirmed one is logged without a pid — nothing that thread spawned can be swept.
- A cell's `timeout` is suspended only by host-side bridge waits (`agent()`/`completion()` handles, `tool.*`), which may run arbitrarily long. That pause is minted and released by the host inside one `try`/`finally` on a dedicated control channel; status events emitted *by* a runtime (`__omp_emit_status__`, an `application/x-omp-status` display bundle) and ordinary tool status events that merely carry the same `op` name can never pause the deadline, so evaluated code cannot outlive its own budget.
